...
router.post("/", async (req, res) => {
    ...
    bodyPix = BodyCompraPixData.BODY_PIX_HOM(dadosUsuario, dadosInscricao, dadosInscricao[0].codigo_referencia, dataFormatada)
    // console.log('bodyPix>',bodyPix);  

    //GERANDO PIX
    axios.post(`${environment.pagSeguroSandBox.ApiPix}`, bodyPix ,options)
    .catch(({ response }) => { 
  ...
  router.post("/validar-pix", async (req, res) => {
    ...
    axios.post(`${environment.pagSeguroSandBox.ApixValidarPagamentoPix}${params.codigo}`,undefined,options)
    .catch(({ response }) => { 
      console.log('RESPONSE1>',response);  
      console.log('RESPONSE2>',response.data);
      let resp = response.data;
    ...
    .then(function (response) {
        ...
        resp !== undefined ? res.send(resp).status(200) : res.send('').status(400);  
    });
